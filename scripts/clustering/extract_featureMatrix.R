...
#helper function
find_centroids <- function(expression_matrix, cluster_var, 
                         surface_markers, signaling_markers, 
                         central_tendency, ...){ 
  my.centroids <- expression_matrix %>% dplyr::group_by(!!sym(cluster_var), add = TRUE) %>% 
    summarize_at(.funs = central_tendency, .vars = vars(one_of(c(surface_markers, signaling_markers))))
  return(my.centroids)
} 
...
find_stim_features <- function(expression_matrix = NULL, cluster_var = NULL, stim_var = NULL, signaling_markers = NULL,
                               central_tendency = NULL, ...){ 
    stim.summaries <- expression_matrix %>% dplyr::group_by(!!sym(stim_var), !!sym(cluster_var), add = TRUE) %>% 
    dplyr::summarize_at(signaling_markers,.funs = central_tendency)
    basal.summaries <- stim.summaries %>% dplyr::filter(!!sym(stim_var) == "Basal") %>% 
      ungroup() %>% dplyr::select(-stimulation)
    my.signals <- which(colnames(basal.summaries) %in% signaling_markers)
    names(basal.summaries)[my.signals] <- paste0(colnames(basal.summaries)[my.signals], "_basal")
    ...
    for(signal in signaling_markers){
      stim.summaries[signal] <- stim.summaries[signal] - stim.summaries[paste0(signal, "_basal")]
    }
    ...
    names(stim.summaries)[my.signals] <- paste0("delta_", colnames(stim.summaries)[my.signals])
    stim.summaries <- stim.summaries %>% ungroup() %>% 
      ...
      tidyr::spread(key = stim.signal, value = `change from basal`, fill = 0) #this fills in NAs with 0
    return(stim.summaries)
}

#actual function
extract_featureMatrix <- function(expression.matrix = NULL, 
                                  surface_markers = NULL, signaling_markers = NULL,
                                  grouping_vars = NULL, cluster_var = NULL, stim_var = NULL, 
                                  central_tendency = mean, ...){
  expression.matrix <- expression.matrix %>% dplyr::ungroup() %>% dplyr::group_by(!!!syms(grouping_vars))
  abundances <- find_abundances(expression.matrix, cluster_var)
  centroids <- find_centroids(expression.matrix, cluster_var, 
                              surface_markers, signaling_markers,
                              central_tendency, ...)
  if(!is.null(stim_var)){
    stim_features <- find_stim_features(expression.matrix, cluster_var, stim_var, signaling_markers, 
                                        central_tendency, ...)
  ...
  if(is.null(grouping_vars)){
    ...
    feature_matrix <- my_features %>% ungroup() %>%
      gather(key = "variable", value = "value", -one_of(c(grouping_vars, cluster_var))) %>% 
      mutate(cluster.variable = paste(!!sym(cluster_var), variable, sep = "_")) %>% 
