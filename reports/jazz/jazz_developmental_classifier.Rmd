...
csv_path <- here::here("reports", "jazz", "out_data")
...
## 4. How do flowSOM metaclusters and developmental clusters relate to one another? 

We can calculate the percentage of each flowSOM metacluster that is made up of cells in each developmental subpopulation: 
...
cluster_comparison_data <- 
  jazz_data %>% 
  rename(developmental_cluster = cluster) %>% 
  filter(developmental_cluster != "Thrombocyte") %>% 
  count(flowsom_metaclusters, developmental_cluster, .drop = FALSE) %>% 
  group_by(flowsom_metaclusters) %>% 
  summarize(
    prop = n / sum(n), 
    developmental_cluster = developmental_cluster
  ) %>% 
...
cluster_comparison_data %>%  
  ggplot(
    aes(x = flowsom_metaclusters, y = percentage, fill = developmental_cluster)
  ) + 
  geom_col(position = "dodge") + 
  #scale_x_discrete(labels = function(x) str_sub(x, end = 5L)) + 
  scale_y_continuous(labels = scales::label_number(suffix = "%")) + 
  #facet_grid(rows = vars(flowsom_metaclusters)) + 
  theme(legend.position = "bottom") + 
  labs(
    x = "Flowsom metacluster ID", 
    y = 
      str_wrap(
        "% of cells in each metacluster assigned to a developmental cluster",
        width = 40
      ), 
    fill = NULL
  )
...
percentage_plot <- 
  cluster_comparison_data %>% 
  ggplot(
    aes(x = 1, y = percentage, fill = developmental_cluster)
  ) + 
  geom_col(position = "dodge") + 
  #scale_x_discrete(labels = function(x) str_sub(x, end = 5L)) + 
  scale_y_continuous(labels = scales::label_number(suffix = "%")) + 
...
Run metacluster centroids through the classifier: 
...
metacluster_centroids <-
  jazz_data %>% 
  group_by(flowsom_metaclusters) %>% 
  summarize(across(all_of(CLASSIFIER_MARKERS), mean))

metacluster_centroid_classifications <- 
  metacluster_centroids %>% 
  group_by(flowsom_metaclusters) %>% 
  tidytof:::tof_classifier_apply(
    classifier_fit = classifier_fit, 
    num_cores = 1, 
    parallel_var = flowsom_metaclusters
  )

distance_plot <- 
  metacluster_centroid_classifications %>% 
  mutate(flowsom_metaclusters = metacluster_centroids$flowsom_metaclusters) %>%
  pivot_longer(
    cols = -c(flowsom_metaclusters, mahalanobis_cluster),
    names_to = "distance", 
    values_to = "value"
  ) %>% 
  group_by(flowsom_metaclusters) %>% 
  mutate(
    value = norm(value), 
    distance = 
      str_remove(distance, "mahalanobis_") %>% 
      factor(levels = CLASSIFIER_POPULATIONS)
  ) %>% 
  ggplot(aes(x = 1, y = value, fill = distance)) + 
  geom_col(position = "dodge") + 
  ...
  labs(
    ...
    y = 
      str_wrap(
        "Normalized distance from flowsom cluster centroid to mahalanobis cluster centroid", 
        width = 40
      ), 
    fill = NULL
  )
...
$$
logit(p_{ij}) = log(\frac{p_{ij}}{1 - p_{ij}}) = \beta_0 + \alpha_i^{(p)} + \alpha_j^{(s)} + \beta_1 X\_{72hr_j} + \beta\_{2} X_{96hr_j}
$$
...
my_markers <- 
  c(
    ...
    "ps6", "cd49f", "hladr", "cd71", "pcreb", "cd11b"
  )
...
### Save differentially abundant clusters

Other people might want to manually investigate the results from this analysis, so we save them below as a .csv. 
...
raw_jazz_data <- 
  ...
  mutate(
    patient = 
      str_extract(string = file_name, pattern = "_[:digit:]+") %>% 
      str_remove("_") %>% 
      replace_na("healthy"), 
    timepoint = 
      str_extract(string = file_name, pattern = "[:digit:]+hr") %>% 
      replace_na("healthy")
  ) %>% 
...
raw_jazz_data <- 
  raw_jazz_data %>% 
  mutate(
    flowsom_metacluster = jazz_data$flowsom_metaclusters, 
    cluster_type = 
      if_else(
        flowsom_metacluster %in% sig_clusters_tim, 
        "significant", 
        "not significant"
      )
  ) %>% 
  left_join(
    inc_or_dec_tibble, 
    by = c("flowsom_metacluster" = "flowsom_metaclusters")
  ) %>% 
  rename(cluster_increase_or_decrease = inc_or_dec) %>% 
  mutate(
    cluster_increase_or_decrease = 
      replace_na(cluster_increase_or_decrease, "not significant")
  )

write_csv(
  x = raw_jazz_data, 
  file = file.path(csv_path, "output_jazz_data.csv")
)

nested_raw_jazz_data <- 
  raw_jazz_data %>% 
  group_by(file_name) %>% 
  nest() %>% 
  mutate(
    file_name = 
      str_replace(file_name, pattern = ".fcs", replacement = ".csv")
  )

map2(
  .x = nested_raw_jazz_data$file_name, 
  .y = nested_raw_jazz_data$data, 
  .f = ~ write_csv(x = .y, file = file.path(csv_path, "ind_files", .x))
)
...
```{r, include = FALSE}
# number of significant differences by marker
ds_results %>% 
  topTable(all = TRUE) %>% 
  as_tibble() %>% 
  mutate(significance = if_else(p_adj < 0.05, "*", "")) %>% 
